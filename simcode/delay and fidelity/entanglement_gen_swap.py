...
class EntanglementGenerationAndSwapping(Application):
    ...
    def generate_epr(self, src: QNode, dst: QNode) :
        ...
        for i in range(0,m1.capacity):
            if m1._storage[i]!= None:
                localepr = m1._storage[i]
                store_time = t - localepr.lastoptime
                if float(str(store_time)) > 0:
                    localepr.w = localepr.w*pow(math.e,-float(str(store_time))/coh)
                    localepr.fidelity = (localepr.w*3+1)/4
                    anotherepr = localepr.othernode.get_memory(localepr.othermemname)._storage[localepr.otherindex]
                    if localepr.fidelity > 0.5:
                        #更新纠缠对信息
                        localepr.lastoptime = anotherepr.lastoptime = t
                        anotherepr.w = localepr.w
                        anotherepr.fidelity = localepr.fidelity
                    else:
                        localepr.othernode.get_memory(localepr.othermemname)._storage[localepr.otherindex]=None
                        m1._storage[i] = None

            if m2._storage[i]!= None:
    ...
    def swapping(self,node:QNode,QM1_name:str,QM2_name:str,idx1:int,idx2:int,request:str):
        ...
        localepr1:BaseEntanglement = QM1._storage[idx1]
        localepr2:BaseEntanglement = QM2._storage[idx2]
        #所经历的存储退相干
        store_time1 = self._simulator.tc - localepr1.lastoptime
        store_time2 = self._simulator.tc - localepr2.lastoptime
        ...
        #计算存储带来的保真度下降，交换带来的保真度下降
        if float(str(store_time1)) >0:
            localepr1.w = localepr1.w*pow(math.e,-float(str(store_time1))/coh)
        if float(str(store_time2)) >0:
            localepr2.w = localepr2.w*pow(math.e,-float(str(store_time2))/coh)
        swap_w = localepr1.w*localepr2.w
        #赋予新纠缠对新的纠缠关系
        leftepr.othernode = localepr2.othernode
        leftepr.othermemname = localepr2.othermemname
        leftepr.otherindex = localepr2.otherindex
        leftepr.lastoptime = self._simulator.tc
        leftepr.w = swap_w
        #更新出生时间为两者之间的最小值
        leftepr.birthtime = min(localepr1.birthtime,localepr2.birthtime)
        ...
        rightepr.lastoptime = self._simulator.tc
        rightepr.birthtime = min(localepr1.birthtime,localepr2.birthtime)
        rightepr.w = swap_w
        #计算新的交换保真度
        rightepr.fidelity = leftepr.fidelity = (3*swap_w+1)/4
        #交换后的保真度太低失效
        if leftepr.fidelity < 0.5:
            localepr1.othernode.get_memory(localepr1.othermemname)._storage[localepr1.otherindex] = None
            localepr2.othernode.get_memory(localepr2.othermemname)._storage[localepr2.otherindex] = None
            #擦除本地存储
        ...
        if leftepr.thisnode.name == request[0] and leftepr.othernode.name == request[1]:
            ...
            leftepr.othernode.fidelity.append(leftepr.fidelity)
            leftepr.thisnode.fidelity.append(leftepr.fidelity)
            return True
